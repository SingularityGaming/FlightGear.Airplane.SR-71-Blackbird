...
headShake = func {
  ...
  if ( (enabled) and ( view_number == 0)) {
    ...
    # y = -0.0005x3 - 0.0050x2 - 0.0089x - 0.0045
    ...
    if (xDivergence_total > xMax){
      xDivergence_total = xMax;
    }
    if (xDivergence_total < xMin){
      xDivergence_total = xMin;
    }
    ...
    if (yDivergence_total >= yMax) {
      yDivergence_total = yMax;
    }
    if (yDivergence_total <= yMin) {
      yDivergence_total = yMin;
    }
    ...
    if (zDivergence_total >= zMax) {
      zDivergence_total = zMax;
    }
    if (zDivergence_total <= zMin) {
      zDivergence_total = zMin;
    }
